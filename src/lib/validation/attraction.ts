/**
 * Server-side validation for the admin Attraction editor
 * (`POST /api/admin/places`, `PATCH /api/admin/places/[id]`, task 16.2).
 *
 * - Implements the field-shape rules required by Wymaganie 32 #2 (name,
 ...
 * - Slug uniqueness (Wym. 32 #6) is a database-level check and stays in the
 *   route handler / Postgres unique index — not in this pure validator.
 * - Other Attraction fields editable in the admin (description, tagi,
 *   wskazówki praktyczne, informacja dojazdowa, status publikacji) are
 *   free-form strings/arrays/booleans without numeric or shape constraints,
 ...
 *
 * The validation surface (name, slug, region, Map_Data) is intentionally
 * identical to `validateRestaurant`. The two functions are kept separate
 * because their callers are different admin endpoints with potentially
 * different future evolution (e.g. Restaurant could grow a phone-format
 * check). For now they share only this module-local layout, not code.
 *
 * Wymagania: 32, 41, 44.
 */
...
/**
 * Shape of the JSON payload accepted by `validateAttraction`. Every field is
 ...
 *
 * Map_Data is flattened the same way `Attraction` stores it on the row
...
export const ATTRACTION_REGIONS = ['orte_area', 'rome'] as const satisfies readonly Region[];
 ...
 * single hyphens. Same rule as `validateApartment` and `validateRestaurant`:
...
/**
 * Validate an inbound attraction payload. Returns `{ ok: true }` on success,
 * otherwise a list of errors.
 *
 * Rules:
 *  - `name`: required, non-empty string after trimming (Wym. 32 #2).
 *  - `slug`: required, non-empty string matching kebab-case `SLUG_RE`
 *    (Wym. 32 #2, #6 — uniqueness is checked at the DB layer).
 *  - `region`: required, must be `'orte_area'` or `'rome'` (Wym. 32 #2).
 *  - Map_Data (Wym. 32 #2, Wym. 41 #1):
 ...
 */
export function validateAttraction(payload: AttractionPayload): ValidationResult {
  ...
  } else if (!(ATTRACTION_REGIONS as readonly string[]).includes(payload.region as string)) {
    errors.push({
      field: 'region',
  ...
  // --- Map_Data: address (Wym. 32 #2, Wym. 41 #1) --------------------------
